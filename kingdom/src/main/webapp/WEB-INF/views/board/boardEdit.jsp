...
<link rel="stylesheet" type="text/css" href="/kingdom/css/user.css">
<script type="text/javascript" src="/kingdom/js/jquery-3.6.0.min.js"></script>
<script type="text/javascript" src="/kingdom/js/w3color.js"></script>
<style type="text/css">
	label.ft14 {
		line-height: 200%;
	}
...
<%--
	서버에게 페이지를 요청하는 방식 ]
		1. GET 방식
			요청 방법 ]
				...
				==> href 속성에 요청할 페이지를 기술하면 된다.
					예 ]
						<a href="/kingdom/main.cnu?id=euns">홈</a>
						
						
				2) 자바스크립트에서 요청하는 방법
					예 ]
						location.href = '/kingdom/main.cnu?id=' + sid;
				...
				==> form 태그의 method 속성에 get이라고 기술하고
					...
					예 ]
						<form method="GET" action="/kingdom/main.cnu" name="frm" id="frm">
				...
				4)
					태그라이브러리로 처리하는 방법
						<c:redirect url="/kingdom/main.cnu">
							<c:praram name="id" value="euns" />
						</c:redirect>
						
				5) 주소표시줄에 직접 입력하는 방법
					http://localhost/kingdom/main.cnu?id=euns
		...
		==> 같이 전송되는 데이터(파라미터)가 주소표시줄에 노출되지 않는 방식이다.
			...
			방법 ]
				<form method="POST" action="/kingdom/main.cnu" name="frm" id="frm">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#dbtn').click(function(){
			$('#lfrm').attr('action', '/kingdom/board/boardDel.cnu');
			$('#lfrm').submit();
		});
...
//			$('.file').last().remove();
		...
		$('.imgFile').click(function(){
			...
			if(confirm(sno + ' ] 파일을 삭제할까요?')){
				$.ajax({
					url:'/kingdom/board/boardImgDel.cnu',
					type: 'post',
					dataType: 'json',
					data: {
						fno: sno
					...
					success: function(obj){
						if(obj.result == 'YES'){
							...
							$(location).attr('href', '/kingdom/member/login.cnu');
						} else {
							alert('*** 삭제에 실패했습니다! ***');
						}
					},
...
<body>
	...
	<form method="POST" action="/kingdom/board/board.cnu" id="lfrm" name="lfrm">
	...
	<div class="w3-content mxw700 w3-margin-top w3-padding">
		...
		<!-- form 태그 -->
		<form method="POST" action="/kingdom/board/boardEditProc.cnu" name="efrm" id="efrm" encType="multipart/form-data"
